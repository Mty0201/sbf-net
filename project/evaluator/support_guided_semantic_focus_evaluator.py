"""Support-guided semantic focus evaluator for the Phase 7 active route.

Reports global semantic metrics, boundary-region semantic metrics, and
support-specific metrics. Does NOT report any dir/dist field metrics.
"""
...
class SupportGuidedSemanticFocusEvaluator:
    """Evaluator contract for the support-guided semantic focus route."""
    ...
    def __call__(self, seg_logits: torch.Tensor, support_pred: torch.Tensor,
        ...
        # Threshold applied to ground-truth support_gt, NOT sigmoid(support_pred)
        ...
        # === 3. Support metrics ===
        # support_bce: BCE on sigmoid probabilities, masked by valid_gt
        ...
        if valid_sum > 0:
            bce_raw = F.binary_cross_entropy(
                prob, support_target, reduction="none"
            )
            support_bce = (bce_raw * valid_gt).sum() / valid_sum.clamp_min(1e-6)
        else:
            support_bce = seg_logits.new_tensor(0.0)

        # support_cover: Tversky overlap on sigmoid probabilities
