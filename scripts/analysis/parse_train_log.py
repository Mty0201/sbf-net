...
Supports two run types, auto-detected from the log content:
  - focus:        SupportGuidedSemanticFocusLoss (active route)
...
def parse_log(log_path: str, run_type: str):
    ...
    with open(log_path, "r") as f:
        for line in f:
            # --- Val/Test batch line (focus type: collect boundary metrics) ---
            if run_type == "focus" and "Val/Test:" in line:
            ...
            if m_val:
                ...
                # Merge boundary metrics (focus type)
                if run_type == "focus":
...
def main():
    ...
    # Determine columns
    columns = FOCUS_COLUMNS if run_type == "focus" else SUPPORT_ONLY_COLUMNS
