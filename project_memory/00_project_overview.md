...
- 当前阶段主线任务: 不再继续扫 support 参数；当前重点是为从架构改进角度重新接入 direction 做进入前准备。`semantic val_mIoU 73.8` 是安全线，`support-only best 74.6` 是当前目标线。
...
- 当前执行优先级: 在 CUDA-enabled `ptv3` 环境中确认 `axis-side` smoke；在 smoke 未确认前，不把该路线写成已通过，也不安排 `axis-side` full train。
