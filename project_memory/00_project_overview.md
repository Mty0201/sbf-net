...
- 当前已确认 `Stage-2 v1` 真实 full train 结果: `SupportConditionedEdgeHead + support(1.0/0.2) + dir(1.0) + dist(0.0)` 最佳 `val_mIoU = 71.34`（epoch 36），最终 `val_mIoU = 68.31`（epoch 100），仍低于 `73.8` 安全线。
...
- 当前已落地一条隔离的 `Stage-2 v2` 最小实验路径: post-backbone `semantic_adapter / boundary_adapter` branch split + 独立 `stage2-v2` config；主线 config 与默认路径保持不变。
