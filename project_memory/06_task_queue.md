...
1. `axis-side` smoke 确认: 在 CUDA-enabled `ptv3` 环境中使用 `semseg-pt-v3m1-0-base-bf-edge-axis-side-train-smoke.py` 确认该路线能完整跑通 train 2 epoch + validate 1 epoch；当前 workspace 仅有一次未完成的启动日志，不能视为 smoke 已通过。
