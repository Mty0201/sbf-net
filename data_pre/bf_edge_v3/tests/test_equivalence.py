...
Phase 4 strategy:
  - Stage 1: still compared against Part A reference (tests/reference/) since
    Stage 1 is unchanged by Phase 4. Uses np.array_equal (strict bit-identical).
  - Stages 2-4: Phase 4 intentionally changes behavior. Old Part A tests are
    @pytest.mark.skip-ed (preserved as documentation). New tests compare against
    Phase 4 reference data (tests/reference_v2/).
  - Cross-checks: validation hooks and in-memory path use Phase 4 pipeline.
"""
...
@pytest.fixture(scope="module")
def phase4_stage2(refactored_stage1):
...
@pytest.fixture(scope="module")
def phase4_stage3(refactored_stage1, phase4_stage2):
    """Run Stage 3 with Phase 4 config."""
...
@pytest.fixture(scope="module")
def phase4_stage4(sample_scene_dir, phase4_stage3):
    """Run Stage 4 with Phase 4 config."""
...
# ---------------------------------------------------------------------------
# Stage 1: boundary centers (unchanged by Phase 4)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Phase 4 equivalence: Stages 2-4 against reference_v2/
# ---------------------------------------------------------------------------


@_SKIP_NO_REF_V2
def test_stage2_local_clusters_v2_identical(phase4_stage2):
    _assert_npz_identical(
        REFERENCE_V2_DIR / "local_clusters.npz",
        phase4_stage2,
        "Stage 2 local_clusters (Phase 4)",
    )


@_SKIP_NO_REF_V2
def test_stage3_supports_v2_identical(phase4_stage3):
    _assert_npz_identical(
        REFERENCE_V2_DIR / "supports.npz",
        phase4_stage3,
        "Stage 3 supports (Phase 4)",
    )


@_SKIP_NO_REF_V2
def test_stage4_edge_dist_v2_identical(phase4_stage4):
    _assert_npy_identical(
        REFERENCE_V2_DIR / "edge_dist.npy",
        phase4_stage4["edge_dist"],
        "Stage 4 edge_dist (Phase 4)",
    )


@_SKIP_NO_REF_V2
def test_stage4_edge_dir_v2_identical(phase4_stage4):
    _assert_npy_identical(
        REFERENCE_V2_DIR / "edge_dir.npy",
        phase4_stage4["edge_dir"],
        "Stage 4 edge_dir (Phase 4)",
    )


@_SKIP_NO_REF_V2
def test_stage4_edge_valid_v2_identical(phase4_stage4):
    _assert_npy_identical(
        REFERENCE_V2_DIR / "edge_valid.npy",
        phase4_stage4["edge_valid"],
        "Stage 4 edge_valid (Phase 4)",
    )


@_SKIP_NO_REF_V2
def test_stage4_edge_support_id_v2_identical(phase4_stage4):
    _assert_npy_identical(
        REFERENCE_V2_DIR / "edge_support_id.npy",
        phase4_stage4["edge_support_id"],
        "Stage 4 edge_support_id (Phase 4)",
    )


# ---------------------------------------------------------------------------
# Cross-check: validation hooks pass on Phase 4 output
# ---------------------------------------------------------------------------


def test_validation_passes_on_phase4_output(
    refactored_stage1,
    phase4_stage2,
    phase4_stage3,
    phase4_stage4,
    ...
    validate_supports(phase4_stage3)
    scene = load_scene(sample_scene_dir)
    validate_edge_supervision(phase4_stage4, num_scene_points=scene["coord"].shape[0])
...
def test_inmemory_path_matches_perstage(sample_scene_dir, phase4_stage3):
