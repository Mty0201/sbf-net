"""CR-Q/CR-V/CR-R data config: grid=0.04 + sphere_rate=0.4 + point_max=80000.

g04 isolation baseline for Phase 8. Derived from clean_reset_data.py.
The ``edge`` key has been removed from InjectIndexValidKeys + Collect in all
three splits — edge.npy is dead code on the g04 path (boundary_mask_r060.npy
and s_weight_r060_r120.npy carry all supervision signal). BFDataset's
conditional edge.npy load (project/datasets/bf.py) makes the file optional.

The three downsampling parameters applied consistently to train/val/test:
...
Intended consumers: pure_bfanet_v4_g04_train.py (CR-Q),
support_weighted_v4_g04_train.py (CR-V), semantic_only_g04_train.py (CR-R).
Reuses the same boundary_mask_r060.npy masks — mask generation is
raw-point-cloud based and grid-independent.
"""
