"""Full training config for semantic + support shape (reg + cover + ordinal)."""
# ======================================================================
...
model = runpy.run_path(
    str(repo_root / "configs" / "semantic_boundary" / "old" / "semseg-pt-v3m1-0-base-bf-edge-model.py")
)["model"]
...
loss = dict(
    ...
    ordinal_weight=0.5,
    ordinal_radius=0.15,
    ordinal_min_dist_diff=0.02,
    ordinal_margin=0.05,
    ordinal_narrowband_min=0.1,
    ordinal_n_samples=512,
    ordinal_max_pairs=2048,
)
evaluator = dict(
...
weight = None
resume = False
work_dir = str(repo_root / "outputs" / "semantic_boundary_support_shape_train")
