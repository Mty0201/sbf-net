...
- 需要先对齐“`2.5` 已结束、当前处于 `Stage-2 architecture rollout / verification phase`”的阶段边界，再交给 worker。
...
1. 先确认 `2.5` 阶段已结束，当前处于 `Stage-2 architecture rollout / verification phase`。
2. 先确认唯一允许主动维护的项目根是 `semantic-boundary-field`；`Pointcept` 仅作为宿主依赖存在，默认只读。
3. 先确认当前 active 主线已切到 `axis + side + support`，`axis-side` train / smoke 是当前验证中心。
4. 若问题疑似来自 `Pointcept`、宿主接口、依赖异常或边界语义冲突，停止继续下发实现任务，并在 brief 中写清停止原因与待分析问题。
5. 提炼当前有效事实、禁改项和允许改动项。
6. 若用户给出的文件名与仓库现状不一致，先做路径映射并写进 brief。
7. 把任务限制为一个最小可交付目标；若工作较大，也要拆成当前轮最小落地点。
8. 只输出 worker 需要的执行信息，不展开新的研究方案。
...
- 不因为当前主线已进入 Stage-2 架构落地核证而放松对主训练入口、主配置、主模型职责的保护。
