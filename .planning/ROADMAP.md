...
- **CR-SD** (DecoupledBFANetSegmentorV1 + CRSDLoss): Subtractive Decoupling — PTv3 backbone + learnable alpha channel split + proj_to_semantic W matrix. Margin stream gets `alpha * feat`, semantic stream gets `feat + W @ (alpha * feat)`. Zero-init W, alpha init 0.5. **Ep54/100 with original Lovász loss: decoupling succeeded** (alpha_mean 0.5→0.003, w_fro 0→5.3, semantic loss 2.69→0.096) **but margin Lovász plateaued at 0.95** — binary Lovász on ~7% positive ratio produces insufficient gradient. Best val_mIoU=0.7198 (−1.4pp vs CR-A). **Loss redesigned (commit 0e4c9a8, 2026-04-12):** margin Lovász → BCE + global Dice (CR-P recipe, aux_weight=1.0); semantic CE → s_weight continuous 5× upweight (core→×5, transition→exponential decay to ×1, background→×1; falls back to hard boundary_mask). Retraining pending. **Display baselines: semantic-only 72.5, BFANet 73.1, our v1 (CR-SD) 73.6.**
- **CR-SDE** (DecoupledBFANetSegmentorGRef + CRSDLoss): CR-SD + CrossStreamFusionAttention (gv4) + v1/v2 dual supervision. **Redesigned (commit 2801ad6, 2026-04-12):** GatedSegRefiner (single-direction marg→seg scalar gate) replaced with BFANet-style Q-fusion bidirectional cross-attention (K=1024 patches). Added v1 heads (pre-fusion) + v2 heads (post-fusion), both supervised with identical loss (s_weight 5× CE + Lovász + BCE + Dice), 1:1 weighting. v1 supervision anchors decoupling MLP gradients; fusion re-aligns diverged streams before prediction, preventing late-stage gradient conflict. BFANet's official ES Attention K=1 is degenerate (softmax(scalar)≡1); our K=1024 is structurally non-trivial. Zero-init output projections guarantee step-0 ≡ CR-SD. **BF grid=0.04 variant committed 2026-04-13 (commit 14df6f5):** `configs/semantic_boundary/clean_reset_g04_s38873367/cr_sde_train.py` = `DecoupledBFANetSegmentorGRef` + `CRSDLoss` + `clean_reset_data_g04.py`, training hyperparams byte-identical to CR-SD (bs=2, grad_accum=12, 2000 epoch, eval_epoch=100, seed=38873367, AdamW lr=0.006/0.0006, OneCycleLR, mix_prob=0.8, AMP). Completes the clean_reset_g04 experiment spectrum — the single config combining all four levers (decoupling + cross-stream fusion + dual supervision + continuous support-weighted CE). Awaiting training.
...
|---|---|---|---|---|---|
| 1 | pure PTv3 (new baseline anchor) | **CR-R** ✅ committed 6055ab8 (quick 260411-tqv) | `clean_reset_data_g04.py` | `clean_reset_semantic_model.py` | `SemanticOnlyLoss` (CE + Lovasz) |
...
| 4 | our weighted dual-branch + g module | **CR-Q** ✅ committed 338c0ce → migrated to g04 dir in 6f9c3b2 (quick 260411-tqv), real-env smoke PASSED | `clean_reset_data_g04.py` | `BoundaryGatedSemanticModelV4` | `DualSupervisionPureBFANetLoss` |
| 4b | CR-Q + continuous support-weighted semantic CE | **CR-V** ✅ committed 4459298/2103cb6/892fa4d → migrated to g04 dir in 6f9c3b2 (quick 260411-tqv), real-env smoke PASSED | `clean_reset_data_g04.py` (adds `s_weight` key) | `BoundaryGatedSemanticModelV4` | `DualSupervisionSupportWeightedBFANetLoss` |
| 5 | legacy model +1 | **CR-U** (reserved, non-blocking) | TBD | TBD (DLA-Net / KPConv / Swin3D) | TBD |

Config naming convention: `<base>_g04_train.py` under `configs/semantic_boundary/clean_reset_g04_s38873367/` (moved from `clean_reset_s38873367/` in quick task 260411-tqv, 2026-04-11 evening, to physically isolate grid=0.04 experiments from the frozen grid=0.06 tree). CR-Q is adopted as the Phase 8 sentinel at slot #4; CR-R slot #1 is materialized.
...
**g iteration history:**
- **g v3** (BoundaryGatingModule, commit fe63163): PTv3 patch self-attention on boundary_feat → per-channel gate → `semantic_feat * (1 + gate)`. Carried in CR-J. **Dropped.**
- **g v4** (CrossStreamFusionAttention, commit 3fdfbd1): K=patch_size fusion-query cross-stream attention. BFANet-style Q-fusion bidirectional cross-attention. Carried in CR-M and CR-P with dual supervision. **Empirically near-identity in CR-P** (v2 − v1 = 0.003-0.007 across 78 epochs at grid=0.06).
- **g v4 in CR-SDE** (commit 2801ad6): same CrossStreamFusionAttention integrated into SubtractiveDecoupling architecture. Replaces GatedSegRefiner (single-direction marg→seg scalar gate). Now used between decoupled streams with v1/v2 dual heads. **Active version.**
...
**GatedSegRefiner status (CR-SDE original):** **Replaced 2026-04-12.** Single-direction marg→seg scalar gate was designed before SubtractiveDecoupling existed; with decoupling providing truly divergent streams, bidirectional fusion is needed to prevent late-stage gradient conflict. CrossStreamFusionAttention (gv4) is the replacement.
