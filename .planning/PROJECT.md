...
- **Active work:** v2.0 Phase 8 — Grid=0.04 range-shift validation sweep. **Root cause of the Phase 5 aux-head 0.725 ceiling identified 2026-04-11** via direct measurement of the real training transform pipeline: at `grid_size = 0.06` only **6.75%** of non-boundary voxels lie within 6 cm of a boundary voxel, `b→nearest-nonb p50 = 6.03 cm = exactly 1 grid step`, and `b→b p50 = 3.47 cm` — the r=0.06 m boundary band is rasterised to ~1 voxel thickness so cross-stream attention K neighbours cannot group it. At `grid=0.04` with the user's validated ScanNet/PTv3 recipe (`SphereCrop rate=0.4`, `point_max=80000`), `nonb<6cm` rises to **10.28% (+52%)**, `b→b p50` drops to 2.54 cm, and the raw boundary ratio stays at 11–12% (no class pollution). Widening the KNN radius r was measured and rejected (at r=0.12, wall coverage inflates from 15% to 33% for +1.26pp density gain). **Grid is the only lever.** Phase 8 opened as the range-shift validation sweep: **CR-R** (pure PTv3 = new baseline anchor), **CR-S** (pure BFANet single-stream control), **CR-T** (our weighted dual-branch = CR-L model at grid=0.04), **CR-Q** (our weighted dual-branch + g module = CR-P at grid=0.04, committed 338c0ce + smoke-validated), **CR-U** (legacy model +1, reserved TBD, non-blocking). Success criterion: `min(best CR-T, best CR-Q) ≥ best CR-R`. All Phase 5 verdicts (CR-L 0.7251 @ ep68, CR-P 0.7241 @ ep45, CR-M/N/O/K closed or queued) are preserved and gated on `grid_size = 0.06` via retrospective banners on `docs/canonical/*.md`, `.planning/phases/05-boundary-proximity-cue-experiment/05-CONTEXT.md`, and project memory — no historical verdict is marked invalidated or disproven. Phase 7 (canonical update + milestone close) is deferred until Phase 8 produces its grid=0.04 verdict. Post-training test hook is still separately broken — blocks test-split numbers but not training verdict; deferred.

## Active Milestone: v2.0 — Semantic-first boundary supervision reboot

Fix integration defects, run controlled CR-A/CR-B comparison, diagnose why support regression fails, and redesign the route. The diagnosis is complete: continuous Gaussian regression produces competing gradient; the concept is valid but the implementation was wrong. The new route reinterprets `valid + support` as a boundary proximity cue and validates via a CR-C→CR-P experiment sweep. The sweep evolved beyond the initial CR-C confidence-weighted BCE design through four waves: (1) CR-G/H tried to rescue continuous regression (BCE then MSE+Dice) — both closed out, with CR-H's full training matching CR-B net negative and proving that no continuous-target loss alone fixes the gradient mismatch; (2) CR-I/J/K added a BFANet-style semantic CE upweight and a boundary→semantic gate (g v3) — CR-J was dropped once g iterated to v4, CR-K remains a pending ablation; (3) CR-L/M moved to a BFANet-style binary threshold (physically bounded by voxel radius) — **CR-L ep100 completed 2026-04-11 with best 0.7251 @ ep68, −0.85pp vs CR-A within single-seed noise, establishing that the single-stream aux route cannot meaningfully beat semantic-only**; CR-M applies the same loss to a dual v1+v2 stream coupled by g v4 cross-stream fusion attention and awaits real-env queue. (4) 2026-04-11 refactor: BFANet radius truth corrected to absolute r = 0.06 m (retracting a misread "3× voxel" framing), preprocessing pipeline for per-chunk boundary masks landed, and three new experiments target faithful BFANet reproduction — CR-N (pure BFANet loss on single-stream model), CR-O (fully soft-weighted semantic CE, no aux head), and CR-P (cross-product = dual-stream × PureBFANetLoss × r=0.06 mask, training on real env). Geometric field objectives remain deferred to Part 2, conditional on Part 1 success. Phase numbering reset to 1. See `.planning/ROADMAP.md` for phase structure and `.planning/REQUIREMENTS.md` for scoped requirements.

**Phase 8 (opened 2026-04-11 — Grid=0.04 range-shift validation sweep).** Direct measurement of the real training transform pipeline established that `grid_size = 0.06` rasterises the r=0.06 m boundary band to ~1 voxel thickness (6.75% of non-boundary voxels within 6 cm; `b→nearest-nonb p50 = 6.03 cm = 1 grid step`), and `grid=0.04` with `rate=0.4`, `point_max=80000` raises that to 10.28% (+52%) without changing the raw boundary ratio. Widening r was measured and rejected. All Phase 5 aux-head verdicts (CR-L 0.7251, CR-P 0.7241, Δ=0.001, both ~1pp below CR-A 0.7336) are preserved and gated on grid=0.06 — none of them is marked invalidated, disproven, or wrong. Phase 8 re-runs the boundary-supervision sweep under a pipeline where the boundary band is mechanically resolvable, through a 5-slot matrix: **CR-R** pure PTv3 (new baseline anchor), **CR-S** pure BFANet, **CR-T** our weighted dual-branch, **CR-Q** our weighted dual-branch + g module (sentinel, committed 338c0ce), **CR-U** legacy +1 (reserved TBD). Success criterion: `min(best CR-T, best CR-Q) ≥ best CR-R`. Phase 7 (canonical update + milestone close) is deferred until Phase 8 produces its grid=0.04 verdict. See `.planning/phases/08-grid04-range-shift-validation/08-CONTEXT.md` for the phase container.
...
- **CUE-01, CUE-02, CUE-03, CUE-04**: Boundary proximity cue experiment sweep — CR-C→CR-P. Implementation for CR-G/H/I/J/K/L/M/N/O/P complete. CR-G and CR-H closed as failed (continuous-target losses do not recover the CR-A gap). CR-J dropped (g v3 superseded by g v4). **CR-L ep100 landed 2026-04-11: best 0.7251 @ ep68, −0.85pp vs CR-A within single-seed noise — closes the single-stream aux question as "cannot meaningfully beat semantic-only".** CR-M committed (3fdfbd1) awaiting real-env queue. CR-N (530f55f), CR-O (a8cae64), CR-P (3d797b0 + alignment pass 3452189 + abe0fc8) all committed. **CR-P training on real env 2026-04-11**; CR-N and CR-O queued. CR-K retained as a pending ablation. Success criterion: at least one variant reaches mIoU ≥ 0.7336 (Phase 5) — **[gated on grid=0.06; re-validation pending in Phase 8]**
- **G04-01, G04-02, G04-03, G04-04, G04-05**: Grid=0.04 range-shift validation sweep — CR-R (pure PTv3 = new baseline anchor), CR-S (pure BFANet single-stream control), CR-T (our weighted dual-branch), CR-Q (our weighted dual-branch + g module, committed 338c0ce as sentinel), CR-U (legacy +1, reserved TBD). Success criterion: `min(best CR-T, best CR-Q) ≥ best CR-R` under the new grid=0.04 pipeline. Phase container: `.planning/phases/08-grid04-range-shift-validation/08-CONTEXT.md` (Phase 8)
- **GEO-01, GEO-02**: Geometric field extension — conditional on CUE-04 success (Phase 6)
- **GUARD-01, GUARD-02, GUARD-03**: Extension boundary, no overstatement, canonical doc update (Phase 7, pending Phase 8)
...
- **Part 1:** Validate boundary proximity cue through the CR-G→CR-P experiment sweep. Continuous-target losses (CR-G BCE, CR-H MSE+Dice) are closed as failed. g v3 (CR-J) is dropped in favour of g v4 (CR-M). **CR-L (ep100 2026-04-11)** is closed: best 0.7251 @ ep68, −0.85pp vs CR-A, within noise. Active validation now rests on CR-M (dual-stream with CR-L loss), CR-N (single-stream pure BFANet loss), CR-O (fully soft-weighted semantic, no aux), and **CR-P (full BFANet reproduction = dual-stream × PureBFANetLoss × r=0.06 mask, training 2026-04-11)**. CR-K remains a pending ablation. Target: mIoU ≥ CR-A (0.7336)
...
| BFANet ScanNet boundary radius is absolute `r = 0.06 m`, not "3× voxel" | Reading `train.py:82-87` in weiguangzhao/BFANet@master shows a radius search at an absolute physical radius, not a voxel-relative KNN. On ScanNet `0.06 = 3 × 0.02` is a coincidence of the mink-path voxel size; octformer uses `r = 0.006` because coords are pre-scaled by /10.25. Porting "3× voxel" to other datasets is wrong. | v2.0 Phase 5 preprocessing: radius-search at r=0.06 m physical, per-chunk precomputed `boundary_mask_r060.npy`, full 264-chunk run at mean 7.19% positive ratio. Memory and canonical docs both corrected. |
| Single-stream boundary-aux route ceilings near CR-B, does not beat CR-A | CR-L ep100 landed best val_mIoU = 0.7251 at ep68 (−0.85pp vs CR-A 0.7336, +0.67pp vs CR-B 0.7184). The 0.85pp gap is within the 0.05–0.07 single-seed noise floor measured from CR-A and CR-L training curves. After 100 epochs the aux-head single-stream architecture cannot meaningfully exceed semantic-only. | v2.0 Phase 5: remaining validation shifts to dual-stream (CR-M, CR-P) and no-aux-head (CR-O) architectures; single-stream CR-L family is closed as exhausted. |
| CR-P v1:v2 weighting is `1:1` per BFANet code, not `0.5:1.0` | BFANet's `train.py:147-188` sums `sem_v1 + dice_v1 + margin_v1 + margin_dice_v1 + sem_v2 + dice_v2 + margin_v2 + margin_dice_v2` with no per-branch scaling. The earlier `0.5:1.0` "v2 is primary, v1 is regularizer" framing was a mechanism interpretation without code basis. | v2.0 Phase 5 CR-P: `v1_weight = v2_weight = 1.0`, `aux_weight = 1.0`, CE normalization `.mean()` over total points (including ignore), BCE surface form `BCEWithLogits` (AMP-safe). |
...
---
*Last updated: 2026-04-11 after CR-L ep100 + BFANet radius truth + CR-N/O/P landing — CR-L closed (best 0.7251 @ ep68, −0.85pp vs CR-A within noise), CR-P training on real env, CR-M/N/O queued, BFANet r=0.06m preprocessing pipeline live, single-stream aux route declared exhausted*
