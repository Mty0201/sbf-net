...
status: "CR-P training on real environment. Three BFANet alignment items landed (3452189 + abe0fc8): aux_weight 0.3→1.0, v1:v2 0.5/1.0→1.0/1.0, CE normalization .mean() over total. BCE surface form reverted to BCEWithLogits (abe0fc8) because F.binary_cross_entropy is AMP-unsafe; gradient/numerical behavior identical via log-sum-exp. Real env boundary_mask_r060 sync done (user copied from dev), CR-P training started successfully. CR-N not yet queued but inherits same aux_weight fix."
stopped_at: "CR-P training running on real env. Awaiting first epoch metrics. CR-N queued for same real env after CR-P completes or in parallel. CR-L ep100 final curve still outstanding."
