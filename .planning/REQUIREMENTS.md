...
- [x] **METRIC-04**: A Variant A2 training config exists with focus_weight=0.15, boundary_threshold=0.1, and 300 eval epochs (total_epoch=6000, eval_epoch=300).
- [x] **METRIC-05**: Variant A2 inherits model, optimizer, scheduler, data, and support loss parameters identically from the active route, with only focus_weight, epoch count, and work_dir changed.
